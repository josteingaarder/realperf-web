...
export async function fetchPendingReviewBenchmarkList(session: ConsoleSession): Promise<ManagedBenchmarkListItem[]> {
  const supabase = await createServerSupabaseClient();
  const { data } = await supabase
    .from('benchmark_results')
    .select(
  ...
  const filteredRows = rows.filter(
    (row) => session.profile.role === 'super_admin' || canReviewManufacturer(session, row.manufacturer_id)
  );

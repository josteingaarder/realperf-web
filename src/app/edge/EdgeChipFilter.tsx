...
export default function EdgeChipFilter({
  chips,
  initialFavorites,
  canSaveCollections,
}: {
  ...
  const handleToggleFavorite = (id: string) => {
    ...
    const item: StoredChipRef = { id, source: 'edge' };
    const wasFavorite = favorites.some((favorite) => favorite.id === id && favorite.source === 'edge');
    const nextFavorites = wasFavorite
      ? favorites.filter((favorite) => !(favorite.id === id && favorite.source === 'edge'))
      : [...favorites, item];
    ...
    <div>
            ...
            >
              ...
              <Link href={`/edge/${chip.id}`} className="block p-6">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div className="text-slate-500">AI TOPS: <span className="text-slate-200 font-medium">{chip.ai_tops}</span></div>
                  <div className="text-slate-500">TDP: <span className="text-slate-200 font-medium">{chip.tdp_watt}W</span></div>
                  {chip.vram_gb > 0 && <div className="text-slate-500">VRAM: <span className="text-slate-200 font-medium">{chip.vram_gb}GB</span></div>}
      ...
      {filtered.length === 0 && (
        <div className="text-center py-20">
          <div className="text-slate-500 text-lg mb-2">No edge chips found</div>
